      ...
      <div className="results-section">
        ...
        {results.map((routeObj, i) => (
          <div className="result-card" key={i}>
            <div className="result-fare">Total Fare: ₹{routeObj.totalFare}</div>
            <div className="result-duration">Total Duration: {routeObj.totalDuration}</div>
            {routeObj.route.map((leg, idx) => (
              <div className="result-leg" key={idx}>
                {leg.train}: {leg.from} → {leg.to} (Fare: ₹{leg.fare})
              </div>
            ))}
          </div>
        ))}
        {results.length === 0 && !loading &&
          <div style={{ color: "#888", marginTop: 24, fontSize: "1.07rem" }}>No results to display.</div>
        }
      </div>
    </>
  );
